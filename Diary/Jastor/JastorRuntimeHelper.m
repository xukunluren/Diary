	...
	while ((attribute = strsep(&state, ",")) != NULL) {
		if (attribute[0] == 'T') {
			...
			return (const char *)[[NSData dataWithBytes:(attribute + 3) length:len - 2] bytes];
                ...
                return (const char *)[@"int" UTF8String];
            }else if(attribute[1] == 'f'){
                return (const char *)[@"float" UTF8String];
            }else if(attribute[1] == 'd'){
                return (const char *)[@"double" UTF8String];
            }else if(attribute[1] == 'c'){
                return (const char *)[@"char" UTF8String];
            }
        }
	}
	return "@";
...
+ (NSString *)propertyTypeForPropertyName:(NSString *)propertyName ofClass:(Class)klass{
    if (!propertyClassByClassAndPropertyName) {
        propertyClassByClassAndPropertyName = [[NSMutableDictionary alloc] init];
    }
    ...
    free(properties);
    //this will support traversing the inheritance chain
    return [self propertyTypeForPropertyName:propertyName ofClass:class_getSuperclass(klass)];
}
@end
